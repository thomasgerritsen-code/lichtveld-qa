...
function syncOutputs(raw,p){
  ...
  outputs.f1.textContent=p.f1.toFixed(2);outputs.f2.textContent=p.f2.toFixed(2);outputs.energy.textContent=p.energy.toFixed(2);outputs.spread.textContent=p.spread.toFixed(2);
  outputs.fx.textContent=raw.fx+'%';outputs.fy.textContent=raw.fy+'%';
}
function setModeUI(){
  $('#photonBtn').classList.toggle('selected',mode==='photon');$('#electronBtn').classList.toggle('selected',mode==='electron');
  $('#ffBtn').classList.toggle('selected',filter==='ff');$('#fffBtn').classList.toggle('selected',filter==='fff');
  $('#ffWrap').style.opacity=mode==='photon'?'1':'.4';$('#ffWrap').style.pointerEvents=mode==='photon'?'auto':'none';
  $('#photonTarget').hidden=mode!=='photon';$('#electronWindow').hidden=mode!=='electron';$('#photonHead').hidden=mode!=='photon';$('#electronHead').hidden=mode!=='electron';
  $('#mlcGroup').hidden=mode!=='photon';$('#jawsGroup').hidden=mode!=='photon';$('#electronApplicator').hidden=mode!=='electron';
...
$('#photonBtn').onclick=()=>{mode='photon';update()};$('#electronBtn').onclick=()=>{mode='electron';update()};
$('#ffBtn').onclick=()=>{filter='ff';update()};$('#fffBtn').onclick=()=>{filter='fff';update()};
$('#dispToggle').onchange=e=>{overlays.disp=e.target.checked;update()};$('#labelsToggle').onchange=e=>{overlays.labels=e.target.checked;update()};
$('#badToggle').onchange=e=>{overlays.bad=e.target.checked;update()};$('#vectorsToggle').onchange=e=>{overlays.vectors=e.target.checked;update()};
$('#resetBtn').onclick=reset;$('#faultBtn').onclick=exampleFault;$('#pauseBtn').onclick=e=>{paused=!paused;e.target.textContent=paused?'Hervat':'Pauzeer'};
...
document.querySelectorAll('.component').forEach(el=>el.addEventListener('click',()=>{
  const d=PART_INFO[el.dataset.part];if(!d)return;$('#partTitle').textContent=d[0];$('#partText').textContent=d[1];$('#partPill').textContent=el.dataset.part;
}));

const particles=[];
for(let i=0;i<110;i++){const c=document.createElementNS('http://www.w3.org/2000/svg','circle');c.setAttribute('r',1.5+(i%3)*.4);c.setAttribute('fill','#dffaff');$('#particles').appendChild(c);particles.push({el:c,seed:(i*.6180339887)%1,speed:.10+(i%11)*.007});}
