...
function addStylesheet(){
  ...
  link.rel='stylesheet';
  link.href='./patient-plane.css?v=1';
...
function buildCard(){
  ...
  card.innerHTML=`<div class="metricsHead"><div><strong>Patient-plane cause → effect</strong><span>Genormaliseerde visualisatie van veldvorm, profiel, steering en secundaire componenten</span></div><span class="metricsNote">educatief · geen dosisberekening</span></div>
...
export function initPatientPlane(controller){
  ...
  const update=()=>{
    ...
    setText(card,'#patientPlaneNote',state.note);
