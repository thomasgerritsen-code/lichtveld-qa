...
function mdRender(extraError=''){
  const badge=mdEl('meetdataBadge'),status=mdEl('meetdataStatus');if(!badge||!status)return;if(extraError){badge.textContent='Import geblokkeerd';badge.className='taskState fail';status.innerHTML='<strong>'+mdEsc(extraError)+'</strong>';return}
  ...
  const changed=!!(meetdataSnapshot.manualGun||meetdataSnapshot.manualTarget);badge.textContent=changed?'Handmatig gewijzigd':'Meetdata geïmporteerd';badge.className='taskState '+(changed?'fail':'done');let txt=`${meetdataSnapshot.system} · zichtbaar in Meetdata VT vastgelegd ${mdFormatTime(meetdataSnapshot.capturedAt)} · Vac Gun ${mdFormat(meetdataSnapshot.gun)} · Vac Target ${mdFormat(meetdataSnapshot.target)}. Waarden ongewijzigd overgenomen.`;if(changed){const parts=[];if(meetdataSnapshot.manualGun)parts.push('Vac Gun');if(meetdataSnapshot.manualTarget)parts.push('Vac Target');txt+=` ${parts.join(' en ')} daarna handmatig gewijzigd in het formulier.`}status.textContent=txt;
}
